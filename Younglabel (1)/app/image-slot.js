...
(() => {
  ...
  const stylesheet =
    ...
    // Published / launched view (no editor bridge): never show the dashed drop
    // ring or the empty-state placeholder — end users see only final imagery.
    ':host(:not([data-editable])) .ring{display:none}' +
    ':host(:not([data-editable])) .empty{display:none!important}' +
    // Controls overlay the image at top-right so the author-declared slot
    // height is unaffected. In edit mode they're ALWAYS visible on a filled
    // slot (not hover-gated); the whole tile also stays a drag-and-drop target.
    // Toggle via display (not opacity) so a transition can't leave them
    // half-shown, and so published view (no [data-editable]) hides them fully.
    '.ctl{position:absolute;top:8px;right:8px;display:none;gap:6px;z-index:5;' +
    '  white-space:nowrap}' +
    ':host([data-filled][data-editable]) .ctl,:host([data-reframe]) .ctl{display:flex}' +
    '.ctl button{appearance:none;border:0;border-radius:999px;padding:6px 12px;cursor:pointer;' +
    '  background:rgba(20,18,15,.74);color:#fff;font:11px/1 system-ui,-apple-system,sans-serif;' +
    '  letter-spacing:.03em;backdrop-filter:blur(8px);box-shadow:0 2px 10px rgba(0,0,0,.28)}' +
    '.ctl button:hover{background:rgba(20,18,15,.96)}' +
